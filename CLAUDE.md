...
It is ~3.6k lines (a figure that only ever grows — don't trust it, and don't reintroduce it into this heading where it will silently rot). It holds the landing page, workspace, chat, config modal, and — defined inline as local components — `DebugConsole` (in-app log viewer that patches through `console.*`), `ReportMockup` (renders `layout` as absolutely-positioned divs), and `LandingPage`. Only `LoginPage`, `MobileNav`, `FeaturesPage`, `DocsPage`, `ContactPage`, `UserAvatar` and `Logo` are extracted to `src/components/`. Expect to work inside this file for most feature work; prefer extracting new surfaces into `src/components/` rather than growing it.
