...
| `audit/` (205 KB, 10 files) | a docs dump | The 2026-08-27 audit's findings register. 33 local branches still carry its identifiers (`audit/SEC-001-security-headers` and the rest). It is a historical record. It *is* undocumented in `CLAUDE.md` - that is a documentation gap for Phase 7, not a reason to move 205 KB of findings. **Superseded 2026-08-29:** moved to `docs/audit/` in Phase 7, then reduced at the owner's request to `docs/audit/README.md`, a one-line index of the 31 identifiers the code actually cites. The ten narrative reports are recoverable from history at `64234fd`. The index was kept rather than deleting outright because the IDs appear in 68 comments across 43 files and in 33 branch names, none of which an edit can update. |
