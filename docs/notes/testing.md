...
`npm test` runs Vitest over **1,157 tests in 53 files** — under the `node` environment
...
| `lib/dataSource` | 29 |  |
| `lib/datetime` | 12 |  |
| `lib/designerBridge` | 45 | 8 from one loop |
| `lib/geminiClient` | 6 |  |
| `lib/geminiErrors` | 14 |  |
...
**Total: 1,157 in 53 files** — and that total is the arithmetic sum of the column above
...
A grep for `\bit(` answers **1,135** against a real 1,157 and will talk you into
