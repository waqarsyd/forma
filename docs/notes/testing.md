...
`npm test` runs Vitest over **1,320 tests in 60 files** — under the `node` environment
...
| `components/BatchPanel` | 11 | jsdom |
...
| `lib/zip` | 24 |  |
| `server/bindHost` | 6 |  |
| `server/hostingConfig` | 6 | reads `firebase.json` off disk |
| `server/securityHeaders` | 27 |  |
| `server/staticCache` | 10 |  |
| `services/chatRetry` | 10 |  |
| `services/geminiService` | 33 | 7 of these come from one loop |
| `services/keyVault` | 21 | jsdom |
| `services/modelResolution` | 32 | jsdom |

**Total: 1,320 in 60 files** — and that total is the arithmetic sum of the column above
...
A grep for `\bit(` answers **1,292** against a real 1,320 and will talk you into
