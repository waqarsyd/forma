...
`npm test` runs Vitest over **1,141 tests in 52 files** — under the `node` environment
...
| `lib/dataSource` | 29 |  |
| `lib/datetime` | 12 |  |
| `lib/designerBridge` | 44 | 8 from one loop |
| `lib/geminiClient` | 6 |  |
| `lib/geminiErrors` | 14 |  |
...
**Total: 1,141 in 52 files** — and that total is the arithmetic sum of the column above
...
A grep for `\bit(` answers **1,119** against a real 1,141 and will talk you into
