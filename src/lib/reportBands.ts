  ...
  return `
          ...
          - WHICH BAND EACH PIECE OF THE DESIGN BELONGS IN.
            ...
            - PageFooter — page numbers (XRPageInfo), the registration line, anything repeated at the foot of every sheet.
              THE PAGE NUMBER GOES HERE AND NOWHERE ELSE, whatever the design looks like. On a one-page source the number is drawn below the totals, which makes ReportFooter look right; it is not. ReportFooter prints once after the last record, so the number would appear on the last sheet only — invisible on the one-page document you are looking at, and wrong the first time anyone prints a long one. If the design shows a page number anywhere, emit a PageFooter band with real height and put the XRPageInfo in it. Do not emit PageFooter at HeightF="0" and place the number somewhere else.
